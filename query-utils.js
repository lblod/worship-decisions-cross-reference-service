...
export async function getRelatedToActiveCKB( eenheidUri ) {
  const queryStr = `
    SELECT DISTINCT ?ckb WHERE {
      ?ckb
        <http://www.w3.org/ns/org#hasSubOrganization> ${sparqlEscapeUri(eenheidUri)} ;
  ...
  const response = await querySudo(`
    ...
    WHERE {
      ?rule
        ...
        lblodBesluit:decidableBy ${sparqlEscapeUri(referrerOrgType)} ;
      ...
      WHERE {
        {
          ...
          WHERE {
            ${fromEenheid ? `VALUES ?eenheid { ${sparqlEscapeUri(fromEenheid)} }` : ''}
            ${forEenheid ? `VALUES ?eredienst { ${sparqlEscapeUri(forEenheid)} }` : ''}
            ${decisionType ? `VALUES ?besluitType { ${sparqlEscapeUri(decisionType)} }` : ''}
            ${forDecision ? `VALUES ?childDecision { ${sparqlEscapeUri(forDecision)} }` : ''}
            ${ckbUri ? `VALUES ?ckb { ${sparqlEscapeUri(ckbUri)} }` : ''}

            FILTER EXISTS {
              ?betrokkenBestuur org:organization ?eredienst .
              ?eenheid ere:betrokkenBestuur ?betrokkenBestuur .
              ?ckb org:hasSubOrganization ?eredienst.
            }
        ...
        ${SCOPE_SUBMISSIONS_TO_ONE_GRAPH ? `GRAPH ?g {`: ''}
          ?mostRecentParentSubmission
            prov:generated / dcterms:relation ?childDecision ;
            nmo:sentDate ?dateSent ;
        ...
        BIND(CONCAT(?childDecisionTypeLabel, " van ", ?eredienstLabel, " als laatste gebundeld en verstuurd door ", ?ckbLabel, " op ", ?niceDateSent) as ?displayLabel)
      }`;
  }
  else {
    query = `
      ...
      CONSTRUCT {
        ?subject a ?what ;
          skos:prefLabel ?displayLabel ;
          nmo:sentDate ?dateSent ;
          formdata:sessionStartedAtTime ?sessionStarted ;
          pav:createdBy ?eredienst ;
          rdfs:seeAlso ?seeAlsoUrl .
        ?eredienst skos:prefLabel ?eredienstLabel .
      }
      WHERE {
        ...
        ${forEenheid ? `VALUES ?eredienst { ${sparqlEscapeUri(forEenheid)} }` : ''}
        ${decisionType ? `VALUES ?besluitType { ${sparqlEscapeUri(decisionType)} }` : ''}
        ${forDecision ? `VALUES ?subject { ${sparqlEscapeUri(forDecision)} }` : ''}
...
export function prepareCKBSearchQuery(fromEenheid, forEenheid, decisionType) {
  return `
    ...
    CONSTRUCT {
      ...
      VALUES ?forEenheid { ${sparqlEscapeUri(forEenheid)} }
      VALUES ?fromEenheid { ${sparqlEscapeUri(fromEenheid)} }
